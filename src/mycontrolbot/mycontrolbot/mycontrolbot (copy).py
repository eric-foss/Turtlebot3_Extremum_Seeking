...
class mycontrolbot(Node):
    ...
    def calculate_vr_vl(self):
        delta = self.omega + self.k *(1/(math.exp(-2*self.Lambda*self.elapsed_time)+0.1))* ((-self.h) - self.z) #big omega
    ...
    def control_turtlebot(self):
        msg = Twist()
        msg.linear.x = (math.exp(-self.Lambda*self.elapsed_time)+0.1)*((self.VR + self.VL) / 2)
